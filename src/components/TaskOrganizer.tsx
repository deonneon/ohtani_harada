    ...
    <div className="space-y-6">
                ...
                <div className="w-24 h-2 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width:
                        areaTasks.length > 0
                          ? `${(completedTasks / areaTasks.length) * 100}%`
                          : '0%',
                    }}
                  />
                </div>
              ...
              <div id={`area-content-${area.id}`} className="p-4">
                {areaTasks.length === 0 ? (
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-sm">
                      Click "Create Task" or "Batch Create" to add tasks to this
                      area.
                    </p>
                  </div>
                ) : (
                  <TaskList
                    ...
                    onReorder={(reorderedTasks) =>
                      handleAreaTaskReorder(area.id, reorderedTasks)
                    }
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {focusAreas.map((area, index) => {
            const areaTasks = tasks.filter((task) => task.areaId === area.id);
            const completedCount = areaTasks.filter(
              (task) => task.status === 'completed'
            ).length;
            const inProgressCount = areaTasks.filter(
              (task) => task.status === 'in-progress'
            ).length;
            const pendingCount = areaTasks.filter(
