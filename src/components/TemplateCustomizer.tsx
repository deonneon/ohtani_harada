...
const TemplateCustomizer: React.FC<TemplateCustomizerProps> = ({
  ...
  const [customizedAreas, setCustomizedAreas] = useState<
  ...
  const handleApplyTemplate = () => {
    ...
    const customizedData: MatrixData = {
      ...
      tasks: templateData.tasks.map((task) => ({
        ...
        areaId: customizedAreas[
          task.areaId === templateData.focusAreas[0].id
            ? 0
            : task.areaId === templateData.focusAreas[1].id
              ? 1
              : task.areaId === templateData.focusAreas[2].id
                ? 2
                : task.areaId === templateData.focusAreas[3].id
                  ? 3
                  : task.areaId === templateData.focusAreas[4].id
                    ? 4
                    : task.areaId === templateData.focusAreas[5].id
                      ? 5
                      : task.areaId === templateData.focusAreas[6].id
                        ? 6
                        : 7
        ]
          ? `area-${Date.now()}-${
  ...
  const updateArea = (
    index: number,
    field: keyof CreateFocusAreaInput,
    value: string
  ) => {
    setCustomizedAreas((prev) =>
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Personalize the goal and focus areas before applying
            </p>
          </div>
          <button
            onClick={onCancel}
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-xs text-gray-500">
                      {
                        templateData.tasks.filter(
                          (t) => t.areaId === templateData.focusAreas[index]?.id
                        ).length
                      }{' '}
                      tasks
                    </span>
          ...
          <div className="mt-8 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 font-medium">
                  {templateData.focusAreas.length}
                </span>
              </div>
              <div>
                ...
                <span className="ml-2 font-medium">
                  {
                    templateData.tasks.filter((t) => t.priority === 'high')
                      .length
                  }
                </span>
              </div>
