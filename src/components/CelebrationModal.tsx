...
export const CelebrationModal: React.FC<CelebrationModalProps> = ({
  ...
  // Generate stable random values for confetti particles
  ...
  const getMilestoneMessage = (milestone: number) => {
    if (milestone === 100)
      return "Congratulations! You've completed your entire goal. This is an incredible achievement!";
    if (milestone >= 75)
      return "Amazing progress! You're in the final stretch of your journey.";
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      ...
      <div className="bg-white rounded-xl p-8 max-w-md w-full mx-4 text-center relative overflow-hidden">
        {/* Animated background */}
        <div className="absolute inset-0 bg-linear-to-br from-yellow-400 via-pink-500 to-purple-600 opacity-10 animate-pulse"></div>
        ...
        <div className="mb-6 relative">
          <div className="w-full bg-gray-200 rounded-full h-4">
            <div
              className="bg-linear-to-r from-green-400 to-blue-500 h-4 rounded-full transition-all duration-1000 ease-out"
        ...
        {/* Motivational quote */}
        <div className="bg-linear-to-r from-blue-50 to-purple-50 rounded-lg p-4 mb-6 relative">
        ...
        <button
          onClick={onClose}
          className="px-6 py-3 bg-linear-to-r from-blue-500 to-purple-600 text-white rounded-lg hover:from-blue-600 hover:to-purple-700 transition-all duration-200 font-semibold shadow-lg hover:shadow-xl transform hover:scale-105"
        >
          Continue Journey
        </button>
      </div>
    ...
    <div className={`relative ${className}`}>
      <div
        className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl shadow-lg transition-all duration-300 ${
          earned
            ? 'bg-linear-to-br from-yellow-400 to-orange-500 text-white transform scale-110'
      ...
      <div className="mt-2 text-center">
        <p
          className={`text-sm font-semibold ${earned ? 'text-gray-900' : 'text-gray-500'}`}
        >
          {title}
        </p>
